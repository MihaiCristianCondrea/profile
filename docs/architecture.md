...
assets/
  css/       global CSS, tokens, Tailwind input/output
...
6. Add or update tests under `__tests__/`.
7. Run `npm run build:ts`, `npm test -- --runInBand`, and `npm run build`.
...
The repository includes a GitHub Pages workflow at `.github/workflows/pages.yml`. It installs dependencies, runs the Jest suite, runs `npm run deploy`, and uploads a prepared `_site/` artifact after generated `assets/js/**` exists. This prevents the ignored generated JavaScript from disappearing during Pages deployment.
