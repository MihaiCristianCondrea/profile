...
Do not edit or commit these files manually. Change TypeScript in `src/`, Tailwind input in `assets/css/tailwind.input.css`, or static assets under `assets/images/`, `assets/icons/`, `assets/data/`, and the root SEO/PWA files instead.
...
`npm run deploy` runs the full build and verifies required SEO files exist.
...
4. Run `npm run deploy`, which regenerates `bundle.js`, `assets/css/tailwind.css`, and `assets/js/**`.
...
That means the repository stays source-first, while the published GitHub Pages artifact still contains every generated runtime file needed by `index.html`.
