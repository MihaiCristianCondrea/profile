...
describe('SiteMetadata.updateForRoute', () => {
  const modulePath = path.resolve(__dirname, '../assets/js/modules/metadata.js');
  ...
  test('applies provided route metadata to document head', () => {
    ...
    expect(document.querySelector('meta[name="description"]').getAttribute('content')).toBe(
      'Detailed showcase of current Android and web experiments.'
    );
    expect(document.querySelector('meta[name="keywords"]').getAttribute('content')).toBe(
      'android case study, jetpack compose ui'
    );
  ...
  test('falls back to defaults when metadata is missing', () => {
    ...
    expect(result.canonicalUrl).toBe(
      'https://mihaicristiancondrea.github.io/profile/#unknown-page'
    );
    expect(document.querySelector('meta[property="og:title"]').getAttribute('content')).toBe(
      'Unknown Page'
    );
