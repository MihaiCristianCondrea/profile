...
import {
  ...
  removeItem,
  updateComplexList
} from '../assets/js/pages/resume.js';
...
describe('pages/resume helpers', () => {
  ...
  test('addListItem inserts a new list entry and updates the resume preview', () => {
    ...
    const previewItems = Array.from(document.querySelectorAll('#resume-skills li')).map((li) => li.innerText);
  ...
  test('addInterestItem supports project flag and updates preview list', () => {
    ...
    expect(document.querySelector('#resume-interests li').innerHTML).toContain('<strong>Project:</strong> Music');
