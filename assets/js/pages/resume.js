...
if (!ModuleRegistry || typeof ModuleRegistry.register !== 'function') {
  throw new Error('Resume page module requires ModuleRegistry.');
}
...
function createInputGroupElement({
  ...
  const inputElement =
    control === 'textarea' ? document.createElement('textarea') : document.createElement('input');
...
function setupRealtimeUpdates() {
  ...
  set(
    'phone',
    (e) =>
      (document.getElementById('resume-phone').innerHTML = e.target.value
        ? `<span class="material-symbols-outlined">call</span><span>${e.target.value}</span>`
        : '')
  );
  set(
    'email',
    (e) =>
      (document.getElementById('resume-email').innerHTML = e.target.value
        ? `<span class="material-symbols-outlined">mail</span><span>${e.target.value}</span>`
        : '')
  );
  set(
    'address',
    (e) =>
      (document.getElementById('resume-address').innerHTML = e.target.value
        ? `<span class="material-symbols-outlined">location_on</span><span>${e.target.value}</span>`
        : '')
  );
  set('summary', (e) => {
...
const addWorkItem = (t, c, s, e, d) =>
  addComplexItem('work', { title: t, company: c, start: s, end: e, desc: d });
const addEducationItem = (d, s, st, e) =>
  addComplexItem('education', { degree: d, school: s, start: st, end: e });
...
function initialize() {
  ...
  document.getElementById('summary').value =
    `Android Developer specializing in the full app lifecycle, from UI/UX design to Google Play publishing, utilizing Jetpack Compose, Kotlin, and Firebase. As a music producer, I bring a creative and user-centric approach to development.`;
  ...
  addComplexItem('work', {
    ...
    desc: `- Android Developer specializing in the full app lifecycle, from UI/UX design to Google Play publishing, utilizing Jetpack Compose, Kotlin, and Firebase. As a music producer, I bring a creative and user-centric approach to development.\n- Launched over 10 Android apps, managing everything from UI/UX design to development and publishing.\n- Focused on a Google-centric design, using Material Design principles for intuitive and consistent apps.`
  });
  addComplexItem('education', {
    degree: 'University',
    school: 'Faculty of Industrial and Robotics Engineering',
    start: '2020',
    end: 'Current'
  });
  addComplexItem('education', {
    degree: 'High School',
    school: 'Hristo Botev Bulgarian Theoretical High School',
    start: '2016',
    end: '2020'
  });
  document.getElementById('photo-preview').style.backgroundImage =
    "url('assets/images/profile/cv_profile_pic.png')";
}
...
ModuleRegistry.register('page.resume', ResumePageModule, { alias: 'ResumePage' });

if (typeof module === 'object' && typeof module.exports === 'object') {
  module.exports = ResumePageModule;
}
