...
async function filterSixteenNineScreenshots(screenshots) {
    ...
    const dimensionMatches = dimensionResults
        ...
        if (!result || !isSixteenNine(result.width, result.height))
            return null;
...
function initProjectsPage() {
    return ensureProjectsMarkedLoaded().then(() => {
        ...
        const animationsApi = typeof SiteAnimations !== 'undefined' && SiteAnimations ? SiteAnimations : null;
        const projectsListEasing = resolveProjectsEasing(animationsApi && animationsApi.easings ? animationsApi.easings.spring : null, 'cubic-bezier(0.4,0,0.2,1)');
        ...
        tabs.forEach((tab, idx) => {
            tab.addEventListener('click', async () => {
                ...
                if (projectsList && projectsList.animate) {
                    ...
                    try {
                        await projectsList.animate([{ opacity: 1, transform: 'translateX(0)' }, { opacity: 0, transform: `translateX(${-20 * dir}px)` }], { duration: 150, easing: projectsListEasing, fill: 'forwards' }).finished;
                    }
                    catch (e) { }
                }
                ...
                if (projectsList && projectsList.animate) {
                    const dir = idx > activeTabIndex ? 1 : -1;
                    try {
                        await projectsList.animate([{ opacity: 0, transform: `translateX(${20 * dir}px)` }, { opacity: 1, transform: 'translateX(0)' }], { duration: 150, easing: projectsListEasing, fill: 'forwards' }).finished;
                    }
                    catch (e) { }
                }
                activeTabIndex = idx;
