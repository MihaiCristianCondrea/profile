...
export const PAGE_ROUTES = Object.create(null);
...
function sanitizeKeywords(value) {
  if (Array.isArray(value)) {
    return value.map((keyword) => (typeof keyword === 'string' ? keyword.trim() : '')).filter(Boolean);
  }
...
function sanitizeOpenGraph(openGraphConfig, route, description) {
  ...
  const title = typeof config.title === 'string' && config.title.trim() ? config.title.trim() : route.title;
  const ogDescription = typeof config.description === 'string' && config.description.trim()
    ? config.description.trim()
    : description;
  const type = typeof config.type === 'string' && config.type.trim() ? config.type.trim() : DEFAULT_OPEN_GRAPH_TYPE;
  const image = typeof config.image === 'string' && config.image.trim() ? config.image.trim() : DEFAULT_SOCIAL_IMAGE;
...
function sanitizeTwitter(twitterConfig, openGraph, description) {
  ...
  const card = typeof config.card === 'string' && config.card.trim() ? config.card.trim() : DEFAULT_TWITTER_CARD;
  const title = typeof config.title === 'string' && config.title.trim() ? config.title.trim() : openGraph.title;
  const twitterDescription = typeof config.description === 'string' && config.description.trim()
    ? config.description.trim()
    : description;
  const image = typeof config.image === 'string' && config.image.trim() ? config.image.trim() : openGraph.image;
  const site = typeof config.site === 'string' && config.site.trim() ? config.site.trim() : DEFAULT_TWITTER_HANDLE;
...
export function sanitizeRouteConfig(config) {
  ...
  if (!normalizedId) {
    throw new Error('Route configuration requires a non-empty "id".');
  }
...
export function registerRoute(config) {
  ...
  if (isUpdate) {
    console.warn(`Route "${sanitized.id}" was overwritten.`);
  }
...
export function registerRoutes(routes) {
  if (!Array.isArray(routes)) {
    throw new TypeError('registerRoutes expects an array of route configurations.');
  }
...
export function hasRoute(routeId) {
  return Boolean(getRoute(routeId));
}
