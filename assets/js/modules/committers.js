...
if (!ModuleRegistry || typeof ModuleRegistry.register !== 'function') {
  throw new Error('Committers module requires ModuleRegistry.');
}
...
const { getDynamicElement } = UtilsModule;
...
function queryElement(id) {
  ...
  const viaUtils = typeof getDynamicElement === 'function' ? getDynamicElement(id) : null;
  if (viaUtils) {
    return viaUtils;
  }
...
async function fetchCommittersRanking() {
  const rankElement = queryElement('committers-rank');
  const statusElement = queryElement('committers-status');
  const updatedElement = queryElement('committers-updated');
  ...
  try {
    ...
    updateCommittersStatus(
      statusElement,
      `Mihai-Cristian Condrea is currently ${formatOrdinal(rank)} in Romania's GitHub committers leaderboard.`,
      false
    );
...
ModuleRegistry.register('committers', CommittersModule, { alias: 'SiteCommitters' });

if (globalScope && typeof globalScope === 'object') {
  globalScope.formatOrdinal = CommittersModule.formatOrdinal;
  globalScope.formatDataAsOf = CommittersModule.formatDataAsOf;
  globalScope.updateCommittersStatus = CommittersModule.updateCommittersStatus;
  globalScope.fetchCommittersRanking = CommittersModule.fetchCommittersRanking;
}

if (typeof module === 'object' && typeof module.exports === 'object') {
  module.exports = CommittersModule;
}
