(function (global) {
  ...
  const hasWaapiSupport = !!(
    ElementConstructor &&
    ElementConstructor.prototype &&
    typeof ElementConstructor.prototype.animate === 'function'
  );
  ...
  function updateMotionTokensForContext() {
    ...
    const reducedDuration =
      typeof reducedReference.reducedDuration === 'number'
  ...
  function computeDistanceValue(baseDistance, context, axis) {
    const base = typeof baseDistance === 'number' ? baseDistance : parseFloat(baseDistance) || 0;
    const windowSize = context && context.windowSize ? context.windowSize : currentWindowSizeClass;
  ...
  function composeTransform(axis, distanceValue, scaleValue) {
    const distance = typeof distanceValue === 'number' ? `${distanceValue}px` : distanceValue;
    const translate =
      axis === 'X' ? `translate3d(${distance}, 0, 0)` : `translate3d(0, ${distance}, 0)`;
    const scale = typeof scaleValue === 'number' && scaleValue !== 1 ? ` scale(${scaleValue})` : '';
  ...
  const LINEAR_EASINGS = Object.freeze({
    accelerate:
      'linear(0, 0.003 3.8%, 0.011 7.7%, 0.025 11.7%, 0.043 15.8%, 0.066 19.9%, 0.092 24.1%, 0.123 28.3%, 0.156 32.5%, 0.193 36.7%, 0.232 40.9%, 0.273 45.1%, 0.316 49.2%, 0.407 57.3%, 0.5 65%, 0.593 72.2%, 0.684 78.9%, 0.768 84.9%, 0.844 90%, 0.957 97.3%, 1)',
    bounce:
      'linear(0, 0.004, 0.016, 0.035, 0.063 9.1%, 0.141, 0.25, 0.391, 0.563, 0.765, 1, 0.891, 0.813 45.5%, 0.785, 0.766, 0.754, 0.75, 0.754, 0.766, 0.785, 0.813 63.6%, 0.891, 1 72.7%, 0.973, 0.953, 0.941, 0.938, 0.941, 0.953, 0.973, 1, 0.988, 0.984, 0.988, 1)',
    decelerate:
      'linear(0, 0.043 1%, 0.092 2.4%, 0.123 3.3%, 0.156 4.4%, 0.193 5.6%, 0.232 7.1%, 0.273 8.7%, 0.316 10.5%, 0.361 12.6%, 0.407 14.8%, 0.453 17.3%, 0.5 20%, 0.547 22.9%, 0.593 26.1%, 0.639 29.5%, 0.684 33.2%, 0.727 37.1%, 0.768 41.4%, 0.807 45.8%, 0.844 50.6%, 0.877 55.7%, 0.908 61.1%, 0.934 66.7%, 0.957 72.7%, 0.975 79%, 0.989 85.7%, 0.997 92.7%, 1)',
    spring:
  ...
  function resolveMotionSpec(meta) {
    ...
    const speed =
      motionMeta.speed === 'fast' || motionMeta.speed === 'slow' ? motionMeta.speed : 'default';

    const requestedScheme =
      motionMeta.scheme && MOTION_SCHEMES[motionMeta.scheme]
        ? motionMeta.scheme
        : getMotionScheme();
    ...
    return {
      ...
      reducedDuration:
        typeof token.reducedDuration === 'number'
  ...
  function resolveDelayForContext(delayValue, motionMeta, context) {
    ...
    const parsedDelay = typeof delayValue === 'number' ? delayValue : parseFloat(delayValue);
    ...
    const motionContext = context && typeof context === 'object' ? context : getMotionContext();
  ...
  function resolveDurationForContext(durationValue, motionMeta, context, motionSpec) {
    ...
    const specDuration =
      motionSpec && typeof motionSpec.duration === 'number' ? motionSpec.duration : durationValue;
  ...
  function getReducedKeyframes(frames) {
    ...
    return [{ opacity: 0 }, { opacity: finalOpacity }];
  }
  ...
  function animateElement(element, keyframes, options, motionMeta) {
    ...
    const framesSource = typeof keyframes === 'function' ? keyframes(motionContext) : keyframes;
    const hasReducedMotion = shouldReduceMotion();
    const framesInput = Array.isArray(framesSource) ? framesSource : [framesSource];
    ...
    if (!hasReducedMotion) {
      const contextualDelay = resolveDelayForContext(
    ...
    const baseEasing =
      animationOptions.easing === undefined ? motionSpec.easing : animationOptions.easing;
    ...
    if (hasReducedMotion) {
      ...
      animationOptions.easing = resolveEasing(
  ...
  function resolveHomeTimeline() {
    ...
    const baseLeadIn = windowSize === 'expanded' ? 150 : windowSize === 'medium' ? 120 : 90;

    let chipsDelay = baseLeadIn;
    let sectionsDelay =
      baseLeadIn + (windowSize === 'expanded' ? 140 : windowSize === 'medium' ? 110 : 95);
    let socialDelay =
      sectionsDelay + (windowSize === 'expanded' ? 110 : windowSize === 'medium' ? 90 : 70);
  ...
  function animateHome(container) {
    ...
    const supportingSections = container.querySelectorAll(
      '.achievement-card, .profile-card-actions, .podcast-embed, .news-section, .contribute-card'
    );
    const socialLinks = container.querySelectorAll('.social-icons a');

    const chipSequence = animateSequence(
      chipElements,
      KEYFRAMES.pop,
      { delay: timeline.chipsDelay },
      timeline.chipGap,
      {
        scheme: 'expressive',
        type: 'spatial',
    ...
    const supportingSequence = animateSequence(
      supportingSections,
      KEYFRAMES.subtleRise,
      { delay: timeline.sectionsDelay },
      timeline.sectionGap,
      {
        scheme: 'expressive',
        type: 'spatial',
    ...
    const socialSequence = animateSequence(
      socialLinks,
      KEYFRAMES.pop,
      { delay: timeline.socialDelay },
      timeline.socialGap,
      {
        scheme: 'expressive',
        type: 'spatial',
  ...
  function animateResume(container) {
    ...
    const formSections = resumePage.querySelectorAll(
      '.form-container .form-section, .form-container h1'
    );
  ...
  function animateProjects(container) {
    ...
    const runIntro = () => {
      ...
      const introAnimation = animateElement(
        intro,
        KEYFRAMES.subtleRise,
        { delay: 60 },
        {
  ...
  function animateDefault(container) {
    ...
    const sectionChildren = container.querySelectorAll(
      '.page-section.active > *:not(script):not(style)'
    );
  ...
  function animatePage(container, pageId) {
    ...
    const targetContainer =
      ElementConstructor && container instanceof ElementConstructor
        ? container
        : root
          ? root.getElementById('pageContentArea')
          : null;
  ...
  function applyMotionEasingCustomProperties() {
    if (
      ...
      !root.documentElement.style ||
      typeof root.documentElement.style.setProperty !== 'function'
    ) {
      return;
    }
  ...
  if (global.ModuleRegistry && typeof global.ModuleRegistry.register === 'function') {
    global.ModuleRegistry.register('animations', global.SiteAnimations, {
      alias: 'SiteAnimations'
    });
  }

  if (typeof module === 'object' && typeof module.exports === 'object') {
    module.exports = global.SiteAnimations;
  }
