...
function initialize() {
    ...
    document.getElementById('summary').value = `A passionate and detail-oriented Android Developer with extensive experience in the entire app lifecycle, from ideation and UI/UX design to publishing on Google Play. Expert in building high-performance, scalable applications using modern technologies like Jetpack Compose, Kotlin, and Firebase. I am also an independent music producer, a background that fuels a creative approach to problem-solving and a keen eye for user experience.`;
    ['name','job-title','phone','email','address','summary'].forEach(id=>document.getElementById(id).dispatchEvent(new Event('input')));
    addListItem('skills','Kotlin, Java, XML, SQL');
    addListItem('skills','Android SDK, Jetpack Compose, MVVM');
    addListItem('skills','Firebase, REST APIs, Room Database');
    addListItem('skills','Git, GitHub, Android Studio');
    addListItem('skills','UI/UX Design, Material Design');
    addListItem('languages','Romanian (Native)');
    addListItem('languages','English (Fluent)');
    addInterestItem('Electronic Music Production');
    addInterestItem('Graphic Design & Animation');
    addInterestItem('This CV was generated by a custom builder I wrote in HTML/JS with Material Design principles.', true);
