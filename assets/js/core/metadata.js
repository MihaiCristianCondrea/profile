...
export function updateForRoute(routeConfig, options = {}) {
  if (!isDomAvailable()) {
    return null;
  }

  const clonedMetadata = cloneMetadata(routeConfig?.metadata);
  const fallbackId = options.pageId || routeConfig?.id || '';
  const fallbackTitle = options.pageTitle || routeConfig?.title || DEFAULT_TITLE;

  const description = clonedMetadata?.description ? clonedMetadata.description : DEFAULT_DESCRIPTION;
  const keywords = toKeywordString(clonedMetadata?.keywords);
  ...
  const openGraph = clonedMetadata?.openGraph || {};
  const ogTitle = typeof openGraph.title === 'string' && openGraph.title.trim() ? openGraph.title : fallbackTitle;
  const ogDescription = typeof openGraph.description === 'string' && openGraph.description.trim()
    ? openGraph.description
    : description;
  const ogType = typeof openGraph.type === 'string' && openGraph.type.trim() ? openGraph.type : DEFAULT_OG_TYPE;
  const ogImage = typeof openGraph.image === 'string' && openGraph.image.trim() ? openGraph.image : DEFAULT_IMAGE;
  ...
  const twitter = clonedMetadata?.twitter || {};
  const twitterCard = typeof twitter.card === 'string' && twitter.card.trim() ? twitter.card : DEFAULT_TWITTER_CARD;
  const twitterTitle = typeof twitter.title === 'string' && twitter.title.trim() ? twitter.title : ogTitle;
  const twitterDescription = typeof twitter.description === 'string' && twitter.description.trim()
    ? twitter.description
    : description;
  const twitterImage = typeof twitter.image === 'string' && twitter.image.trim() ? twitter.image : ogImage;
  const twitterSite = typeof twitter.site === 'string' && twitter.site.trim() ? twitter.site : DEFAULT_TWITTER_HANDLE;
